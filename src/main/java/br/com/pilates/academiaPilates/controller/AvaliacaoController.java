...
public class AvaliacaoController {
    ...
    // public Iterable<Avaliacao>
    // listarAvaliacaoPorIdCliente(@PathVariable("idCliente") Long idCliente) {
    ...
    @PostMapping(path = "${baseUrl}/sistema/api/avaliacao/{idCliente}", headers = "content-type=application/x-www-form-urlencoded , application/json, multipart/*", produces = {
            MediaType.APPLICATION_JSON_VALUE })
    public ModelAndView salvarAvaliacao(Avaliacao avaliacao, BindingResult result,
            @PathVariable("idCliente") Long idCliente, RedirectAttributes atributes) {
        ...
        try {
            ...
            // Sistema
            // cliente.setIdCliente(null); // Setando ID Nulo para criar um novo usuário
            // (Garantir)
            ...
            // atributes.addFlashAttribute("mensagem", "Cliente - " + cliente.getIdCliente()
            // + " " + cliente.getNome() + " editado com sucesso.");
            return mv;
        } catch (Exception e) {
    ...
    public ModelAndView editarAvaliacao(Avaliacao avaliacao, @PathVariable("idAvaliacao") Long idAvaliacao,
            GaleriaFoto galeriaFoto, BindingResult result, RedirectAttributes atributes) {
        ...
        try {
            ...
            // ModelAndView mv = new ModelAndView("redirect:" + baseUrl +
            // "/sistema/cliente/cadastro/" + avaliacao.getCliente().getId());
            ModelAndView mv = new ModelAndView("relatorio/mostrarCadastroPage");
            Iterable<Cliente> listaClientes = ar.findAll();
            mv.addObject("listaClientes", listaClientes);
            // mv.addObject("avaliacao", avaliacao);
            ...
            mv.addObject("listaAvaliacao", listaAvaliacao);
            mv.addObject("abrirModalAposSalvarAvaliacao", true);
            // atributes.addFlashAttribute("mensagem", "Avaliaçãoo - " +
            // avaliacao.getCliente().getNome() + " editado com sucesso!");
            return mv;
        } catch (Exception e) {
    ...
    @DeleteMapping(path = "${baseUrl}/sistema/api/avaliacao", consumes = { MediaType.APPLICATION_FORM_URLENCODED_VALUE,
    ...
    public ModelAndView salvarFotosAvaliacao(MultipartHttpServletRequest request,
            @PathVariable("idAvaliacao") Long idAvaliacao) throws IOException, ServletException {
        ...
        try {
            ...
            // System.out.println("AVALIACAO ANTES _ " + avaliacao.getId() +
            // avaliacao.getFotos().get(0));
            //
            // System.out.println("TAMANHO = " + avaliacao.getFotos().size());
            // System.out.println("AVALIACAO DEPOIS _ " + avaliacao.getId() +
            // avaliacao.getFotos().get(0));
            ...
            ModelAndView mv = new ModelAndView(
                    "redirect:" + baseUrl + "/sistema/cliente/cadastro/" + avaliacao.getCliente().getId());
            Iterable<Cliente> listaClientes = ar.findAll();
            mv.addObject("listaClientes", listaClientes);
            mv.addObject("avaliacao", avaliacao);
            mv.addObject("cliente", cliente);
