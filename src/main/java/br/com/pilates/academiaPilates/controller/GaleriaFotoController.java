...
public class GaleriaFotoController {
    ...
    @RequestMapping(path = "${baseUrl}/sistema/api/galeriaFoto", method = RequestMethod.POST, consumes = {MediaType.APPLICATION_FORM_URLENCODED_VALUE, MediaType.APPLICATION_JSON_VALUE}, headers = "content-type=application/x-www-form-urlencoded , application/json", produces = {MediaType.APPLICATION_JSON_VALUE})
    public ModelAndView salvarFotos(@Valid GaleriaFoto galeriaFoto, BindingResult result, RedirectAttributes atributes) {
        try {
            if (result.hasErrors()) {
